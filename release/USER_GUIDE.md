...
|----------|-------------|----------------|
| `RIDER_WEIGHT_KG` | Body weight in kilograms | Weigh yourself |
| `RIDER_FTP_W` | Cycling FTP in watts | FTP test or Strava power curve estimate |
| `RIDER_MAX_HR` | Cycling maximum heart rate | Max effort or `220 - age` |
| `RIDER_LTHR` | Cycling Lactate Threshold HR (optional) | 20-min all-out test; unlocks LTHR-based HR zones |
| `RIDER_REST_HR` | Resting heart rate (optional, default `60`) | First thing in the morning |
| `RUNNER_RFTP_W` | Running FTP in watts (optional) | Running power meter; falls back to `RIDER_FTP_W` |
| `RUNNER_MAX_HR` | Running max HR (optional) | Usually 5–10 bpm higher than cycling; falls back to `RIDER_MAX_HR` |
| `RUNNER_LTHR` | Running LTHR (optional) | Falls back to `RIDER_LTHR`; unlocks LTHR running zones |
...
| `RIDER_FTP_W` | Cycling: IF, TSS, Power Zones (7-zone), Power Skills |
| `RIDER_MAX_HR` | HR Zones (fallback model), Relative Effort (TRIMP), Heart Points |
| `RIDER_LTHR` | LTHR-based cycling HR zones (more accurate than maxHR%) |
| `RIDER_REST_HR` | More accurate TRIMP calculation |
| `RUNNER_RFTP_W` | Running: IF, TSS, Power Zones (falls back to `RIDER_FTP_W`) |
| `RUNNER_MAX_HR` | Running-specific HR Zones / TRIMP (falls back to `RIDER_MAX_HR`) |
| `RUNNER_LTHR` | LTHR-based running HR zones (falls back to `RIDER_LTHR`) |
...
RIDER_FTP_W=250
RIDER_MAX_HR=195
RIDER_LTHR=165            # Unlocks LTHR-based HR zones
# RIDER_REST_HR=50

# ─── Running Overrides (optional, fall back to cycling values) ───
# RUNNER_RFTP_W=300
# RUNNER_MAX_HR=200
# RUNNER_LTHR=173
...
| No IF/TSS/W/kg metrics | Set `RIDER_FTP_W` and `RIDER_WEIGHT_KG` in `.env`. For running, optionally set `RUNNER_RFTP_W` |
| No LTHR-based HR zones | Set `RIDER_LTHR` (cycling) or `RUNNER_LTHR` (running); falls back to % of max HR |
