...
Pick an activity → automatically fetches, crunches, sends to AI, and updates Strava. Loops back to activity list after each update. Press `q` to quit.
...
**Note:** Set `RIDER_WEIGHT_KG`, `RIDER_FTP_W`, `RIDER_MAX_HR`, `RIDER_LTHR` in `.env` to unlock advanced metrics. For running, optionally set `RUNNER_RFTP_W`, `RUNNER_MAX_HR`, `RUNNER_LTHR`.
...
- AI writes: activity score (Pogačar/Kipchoge/Workout), summary, performance verdict, detailed analysis (pacing, HR, power, training load, zones, climbing, cadence, segments), actionable tips
- Saves the markdown analysis

**Output:** `analysis/*_analysis.md`

**Note:** Requires one of: `GEMINI_API_KEY`, `GROQ_API_KEY`, `OPENROUTER_API_KEY`, or `OPENAI_API_KEY` in `.env`. Auto-detected in that order.
