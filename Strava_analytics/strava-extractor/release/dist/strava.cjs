...
async function getAccessToken(clientId, clientSecret, refreshToken) {
  console.log("\u{1F511} Authenticating with Strava...");
  ...
  const expiresDate = new Date(expires_at * 1e3);
  console.log(`\u2705 Token obtained (expires ${expiresDate.toLocaleString()})`);
  console.log("\u{1F464} Fetching athlete profile...");
  const athleteRes = await axios_default.get(
  ...
  client.interceptors.response.use(
    ...
    async (error) => {
      if (error.response?.status === 429) {
        ...
        const waitTime = 6e4;
        console.warn(`\u26A0\uFE0F  Rate limited by Strava. Waiting ${waitTime / 1e3}s before retrying...`);
...
async function fetchEnrichedActivity(client, activityId, activityName) {
  await rateLimitDelay();
  const detailRes = await client.get(`/activities/${activityId}`, {
  ...
  try {
    await rateLimitDelay();
    const lapsRes = await client.get(`/activities/${activityId}/laps`);
    laps = lapsRes.data;
  } catch (err) {
  ...
  try {
    await rateLimitDelay();
    const zonesRes = await client.get(`/activities/${activityId}/zones`);
    zones = zonesRes.data;
  } catch (err) {
  ...
  try {
    await rateLimitDelay();
    const streamsRes = await client.get(`/activities/${activityId}/streams`, {
      params: {
...
function bestRollingAvg(values, windowSec) {
...
function crunchActivity(raw, rider) {
  ...
  const laps = raw.laps || [];
  const stream4 = raw.stream_data || [];
  const rows = stream4;
  const totalPoints = rows.length;
  const movingRows = rows.filter((r) => r.is_moving !== false);
  const hrValues = movingRows.map((r) => r.heartrate_bpm).filter((v) => v != null);
  const speedValues = movingRows.map((r) => r.speed_kmh).filter((v) => v != null);
  const powerValues = movingRows.map((r) => r.power_watts).filter((v) => v != null);
  const cadenceValues = movingRows.filter((r) => r.cadence_rpm != null && r.cadence_rpm > 0).map((r) => r.cadence_rpm);
  const altitudeValues = rows.map((r) => r.altitude_meters).filter((v) => v != null);
  const gradeValues = movingRows.map((r) => r.grade_percent).filter((v) => v != null);
  const tempValues = rows.map((r) => r.temperature_c).filter((v) => v != null);
  ...
  if (powerValues.length > 30) {
    const pArr = movingRows.map((r) => r.power_watts || 0);
    const rolling = [];
    let wSum = 0;
    for (let i = 0; i < pArr.length; i++) {
      wSum += pArr[i];
      if (i >= 30) wSum -= pArr[i - 30];
      if (i >= 29) rolling.push(wSum / 30);
    }
    normalizedPower = Math.round(Math.pow(rolling.reduce((s, v) => s + v ** 4, 0) / rolling.length, 0.25));
  }
  let pogacarScore = null;
  if (isRide) {
    let speedRef, refLabel;
    if (isVirtual) {
      speedRef = 0;
      refLabel = "Virtual Ride (power only)";
    } else if (elev > 1500) {
      speedRef = 24;
      refLabel = "Race (Mountain Stage)";
    } else if (elev > 500) {
      speedRef = 30;
      refLabel = "Hilly Ride";
    } else if (avgHR > 155) {
      speedRef = 41.5;
      refLabel = "Race (Flat/Rolling)";
    } else {
      speedRef = 34;
      refLabel = "Solo Training";
    }
    const metrics = {};
    let total = 0, count = 0;
  ...
  const summaryCard = {
    ...
    avg_speed: (isRun || isWalk) && summary.average_speed_kmh > 0 ? `${formatPace(summary.average_speed_kmh)} (${summary.average_speed_kmh} km/h)` : isSurf ? `${summary.average_speed_kmh} km/h (includes paddling)` : `${summary.average_speed_kmh} km/h`,
    max_speed: `${summary.max_speed_kmh} km/h`,
    elevation: `${elev} m gained`
  };
  if (tempValues.length > 0) summaryCard.temperature = `${round(tempValues.reduce((a, b) => a + b, 0) / tempValues.length)}\xB0C avg (${Math.min(...tempValues)}-${Math.max(...tempValues)}\xB0C)`;
  ...
  const splitsFormatted = splits.map((s) => ({
    km: s.split,
    speed_kmh: round(s.average_speed * 3.6),
    hr: s.average_heartrate ? Math.round(s.average_heartrate) : null,
    elevation_diff_m: s.elevation_difference,
    moving_time: formatTime(s.moving_time)
  }));
  const sortedBySpeed = [...splitsFormatted].filter((s) => s.speed_kmh > 0).sort((a, b) => b.speed_kmh - a.speed_kmh);
  ...
  const avgS = (arr) => arr.length > 0 ? round(arr.reduce((s, x) => s + x.speed_kmh, 0) / arr.length) : null;
  const avgH = (arr) => {
    const v = arr.filter((x) => x.hr);
    return v.length > 0 ? Math.round(v.reduce((s, x) => s + x.hr, 0) / v.length) : null;
  };
  ...
  for (let t = 0; t < maxTime; t += 300) {
    const wr = rows.filter((r) => r.time_seconds >= t && r.time_seconds < t + 300 && r.is_moving !== false);
    if (wr.length === 0) continue;
    const a = (f) => {
      const v = wr.map((r) => r[f]).filter((x) => x != null && typeof x === "number");
      return v.length > 0 ? round(v.reduce((a2, b) => a2 + b, 0) / v.length) : null;
    };
    timeWindows.push({
  ...
  const fastW = timeWindows.length > 0 ? timeWindows.reduce((b, w) => (w.avg_speed_kmh || 0) > (b.avg_speed_kmh || 0) ? w : b, timeWindows[0]) : null;
  const slowW = timeWindows.length > 0 ? timeWindows.reduce((b, w) => (w.avg_speed_kmh || Infinity) < (b.avg_speed_kmh || Infinity) ? w : b, timeWindows[0]) : null;
  ...
  if (altitudeValues.length > 0) {
    ...
    for (let i = 1; i < altitudeValues.length; i++) {
      const d = altitudeValues[i] - altitudeValues[i - 1];
      if (d > 0) asc += d;
      else desc += Math.abs(d);
    }
    const up = movingRows.filter((r) => r.grade_percent != null && r.grade_percent > 2);
    const dn = movingRows.filter((r) => r.grade_percent != null && r.grade_percent < -2);
    const fl = movingRows.filter((r) => r.grade_percent != null && Math.abs(r.grade_percent) <= 2);
    const tot = up.length + dn.length + fl.length;
    const av = (arr, f) => {
      const v = arr.map((r) => r[f]).filter((x) => x != null);
      return v.length > 0 ? round(v.reduce((a, b) => a + b, 0) / v.length) : null;
    };
    const hardest = splitsFormatted.filter((s) => s.elevation_diff_m > 10).sort((a, b) => b.elevation_diff_m - a.elevation_diff_m)[0] || null;
    climbing = {
      total_ascent_m: Math.round(asc),
      total_descent_m: Math.round(desc),
      ...
      uphill_avg_speed: av(up, "speed_kmh"),
      flat_avg_speed: av(fl, "speed_kmh"),
      downhill_avg_speed: av(dn, "speed_kmh"),
      uphill_avg_hr: av(up, "heartrate_bpm") ? Math.round(av(up, "heartrate_bpm")) : null,
      flat_avg_hr: av(fl, "heartrate_bpm") ? Math.round(av(fl, "heartrate_bpm")) : null,
  ...
  if (powerValues.length > 0) {
    const pArr = movingRows.map((r) => r.power_watts || 0);
    for (const [d, l] of [[5, "5s"], [10, "10s"], [30, "30s"], [60, "1min"], [300, "5min"], [600, "10min"], [1200, "20min"]]) {
  ...
  if (hrValues.length > 0) {
    const hArr = movingRows.map((r) => r.heartrate_bpm || 0);
    for (const [d, l] of [[60, "1min"], [300, "5min"], [600, "10min"], [1200, "20min"]]) {
      const v = bestRollingAvg(hArr, d);
      if (v) peakHR[l] = `${v} bpm`;
    }
  }
  const prSegs = segments.filter((s) => s.pr_rank === 1);
  const allSorted = [...segments].sort((a, b) => (a.pr_rank || 99) - (b.pr_rank || 99) || b.distance - a.distance).slice(0, 15);
  const segTable = allSorted.map((s) => ({
    name: s.name,
    distance: `${round(s.distance / 1e3, 2)} km`,
    time: formatTime(s.elapsed_time),
    ...
    pr: s.pr_rank === 1 ? "\u{1F947} PR #1" : s.pr_rank === 2 ? "\u{1F948} #2" : s.pr_rank === 3 ? "\u{1F949} #3" : "\u2014"
  }));
  ...
  if (hasPowerMeter && normalizedPower && rider.ftpW && rider.ftpW > 0) {
    ...
    else if (IF < 0.9) ifInterpretation = "Tempo / sweetspot effort";
    else if (IF < 1) ifInterpretation = "Threshold effort";
  ...
  if (effectiveMaxHr && hrValues.length > 60) {
    ...
    let ptsPerMin = 0;
    if (isWalk) ptsPerMin = 1;
    else if (isRun) ptsPerMin = 2;
    else if (isRide) ptsPerMin = 1.5;
    else if (isSurf) ptsPerMin = 1.5;
    if (ptsPerMin > 0) {
  ...
  if (hasPowerMeter && hrValues.length > 100) {
    ...
    const avgField = (arr, field) => {
      const vals = arr.map((r) => r[field]).filter((v) => v != null && typeof v === "number");
  ...
  if (hasPowerMeter && rider.weightKg && rider.weightKg > 0) {
    ...
    if (twentyMinWkg >= 6) level = "World Tour Pro";
    else if (twentyMinWkg >= 5) level = "Cat 1 / Elite";
    else if (twentyMinWkg >= 4) level = "Cat 2-3 / Strong Amateur";
    else if (twentyMinWkg >= 3) level = "Cat 4 / Intermediate";
    else if (twentyMinWkg >= 2) level = "Recreational";
    else level = "Beginner";
    powerToWeight = {
  ...
  if (hasPowerMeter && Object.keys(bestPowerRaw).length >= 3) {
    ...
    const sprintScore = round(sprint5s / ftp * 100, 0);
    const attackScore = round(attack1m / ftp * 100, 0);
    const climberScore = round(sustained20m / ftp * 100, 0);
    const ttScore = round(sustained5m / ftp * 100, 0);
    const scores = [
  ...
  if (hrValues.length > 100 && effectiveMaxHr) {
    const zones = [
      { zone: "Z1 (Recovery)", min: 0, max: 0.6 },
      { zone: "Z2 (Endurance)", min: 0.6, max: 0.7 },
      { zone: "Z3 (Tempo)", min: 0.7, max: 0.8 },
      { zone: "Z4 (Threshold)", min: 0.8, max: 0.9 },
      { zone: "Z5 (VO2max+)", min: 0.9, max: 2 }
    ];
    const zoneData = zones.map((z) => {
      const lower = Math.round(z.min * effectiveMaxHr);
      const upper = z.max < 2 ? Math.round(z.max * effectiveMaxHr) : 999;
      const seconds = hrValues.filter((hr) => hr >= lower && (z.max >= 2 ? true : hr < upper)).length;
      return {
        zone: z.zone,
  ...
  if (hasPowerMeter && rider.ftpW && rider.ftpW > 0) {
    ...
    const zones = [
      ...
      { zone: "Z3 (Tempo)", min: 0.75, max: 0.9 },
      { zone: "Z4 (Threshold)", min: 0.9, max: 1.05 },
      { zone: "Z5 (VO2max)", min: 1.05, max: 1.2 },
      { zone: "Z6 (Anaerobic)", min: 1.2, max: 1.5 },
      { zone: "Z7 (Neuromuscular)", min: 1.5, max: 100 }
    ];
    const zoneData = zones.map((z) => {
  ...
  if (gradeValues.length > 100) {
    ...
    if (segments.length > 0) {
      const withGrade = segments.filter((s) => s.segment?.average_grade != null);
      if (withGrade.length > 0) {
        const steepest = withGrade.sort((a, b) => Math.abs(b.segment.average_grade) - Math.abs(a.segment.average_grade))[0];
        steepestSegment = { name: steepest.name, grade: `${steepest.segment.average_grade}%`, distance: `${round(steepest.distance / 1e3, 2)} km` };
      }
    }
    gradientAnalysis = { distribution, steepest_segment: steepestSegment };
  }
  let vamAnalysis = null;
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (inClimb && (r.grade_percent <= 0 || i === rows.length - 1)) {
        ...
        if (duration >= 60 && altGain > 10) {
          ...
          const startDist = rows[Math.max(0, rows.findIndex((x) => x.time_seconds >= climbStart))]?.distance_meters || 0;
          climbs.push({
            start_km: round(startDist / 1e3, 1),
  ...
  return {
    ...
    power: hasPowerMeter ? {
      ...
      weighted_avg_power: normalizedPower,
      // alias
    ...
    best_efforts_running: bestEfforts.length > 0 ? bestEfforts.map((e) => ({ name: e.name, time: formatTime(e.elapsed_time), pr: e.pr_rank === 1 ? "\u{1F947} PR" : e.pr_rank ? `#${e.pr_rank}` : "\u2014" })) : null,
    laps: laps.length > 1 ? laps.map((l) => ({ lap: l.lap_index, distance_km: round(l.distance / 1e3, 2), time: formatDuration(l.moving_time), speed_kmh: round(l.average_speed * 3.6), avg_hr: l.average_heartrate ? Math.round(l.average_heartrate) : null })) : null,
...
function activityLabel(cat) {
  switch (cat) {
    case "walk":
      return { summary: "WALK SUMMARY", emoji: "\u{1F6B6}", speedLabel: "Avg Pace" };
    case "run":
      return { summary: "RUN SUMMARY", emoji: "\u{1F3C3}", speedLabel: "Avg Pace" };
    case "ride":
      return { summary: "RIDE SUMMARY", emoji: "\u{1F6B4}", speedLabel: "Avg Speed" };
    case "surf":
      return { summary: "SURF SESSION", emoji: "\u{1F3C4}", speedLabel: "Avg Speed" };
    default:
      return { summary: "ACTIVITY SUMMARY", emoji: "\u{1F3C5}", speedLabel: "Avg Speed" };
  }
}
function buildDescription(crunched, analysisText) {
  const lines = [];
  ...
  if (crunched.pogacar_score?.composite_pct) {
    ...
    lines.push(`\u{1F3C6} POGA\u010CAR SCORE: ${ps.composite_pct}% (${ps.reference})`);
  ...
  if (crunched.kipchoge_score?.pct) {
    ...
    lines.push(`\u{1F3C6} KIPCHOGE SCORE: ${ks.pct}%`);
    lines.push(`  Pace: ${ks.your_pace} (Kipchoge: ${ks.kipchoge_pace})`);
    lines.push(``);
  }
  if (cat === "surf" && crunched.surf_analysis) {
    const sa = crunched.surf_analysis;
    lines.push(`\u{1F3C4} WAVE REPORT`);
    lines.push(`\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501`);
    lines.push(`  \u{1F30A} Waves caught:   ~${sa.wave_count}`);
    lines.push(`  \u{1F680} Max wave speed: ${sa.max_wave_speed_kmh} km/h`);
    ...
    lines.push(`  \u{1F3C4} Riding time:    ${sa.riding_time_formatted} (${sa.ride_pct}%)`);
    lines.push(`  \u{1F3CA} Paddling time:  ${sa.paddling_time_formatted} (${sa.paddle_pct}%)`);
  ...
  if (sc) {
    lines.push(`\u{1F4CA} ${label.summary}`);
    lines.push(`\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501`);
    if (sc.type) lines.push(`${label.emoji} Type:       ${sc.type}`);
    if (sc.date) lines.push(`\u{1F4C5} Date:       ${sc.date}`);
    if (sc.distance) lines.push(`\u{1F4CF} Distance:   ${sc.distance}`);
    ...
    if (sc.max_speed) lines.push(`\u{1F51D} Max Speed:  ${sc.max_speed}`);
    if (sc.elevation) lines.push(`\u26F0\uFE0F Elevation:  ${sc.elevation}`);
    if (sc.temperature) lines.push(`\u{1F321}\uFE0F Temp:       ${sc.temperature}`);
    if (sc.avg_hr) lines.push(`\u2764\uFE0F Avg HR:     ${sc.avg_hr}`);
    if (cat === "ride" && sc.avg_power) lines.push(`\u{1F9B5} Avg Power:  ${sc.avg_power}`);
    if (cat === "ride" && sc.cadence) lines.push(`\u{1F504} Cadence:    ${sc.cadence}`);
    if (sc.calories) lines.push(`\u{1F525} Calories:   ${sc.calories}`);
    if (sc.gear) lines.push(`\u{1F45F} Gear:       ${sc.gear}`);
    lines.push(`\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501`);
  }
  if (cat === "ride") {
  ...
  if (crunched.heart_points) {
    ...
    lines.push(``);
    lines.push(`\u{1F49A} HEART POINTS: ${hp.points} (${hp.pct_of_weekly_target} of weekly 150 target)`);
  ...
  if (analysisText) {
    ...
    if (verdict) {
      lines.push(``);
      lines.push(`\u{1F4C8} PERFORMANCE VERDICT`);
  ...
  if (analysisText) {
    ...
    if (zones) {
      lines.push(``);
      lines.push(`\u{1F3AF} TRAINING ZONES`);
      lines.push(zones.trim());
    }
  }
  if (analysisText) {
    const sections = cat === "surf" ? [
      { keyword: "Wave", emoji: "\u{1F30A}" },
      { keyword: "Pacing", emoji: "\u{1F4C8}" },
      { keyword: "Heart Rate", emoji: "\u2764\uFE0F" },
      { keyword: "Training Load", emoji: "\u{1F3CB}\uFE0F" },
      { keyword: "Temperature", emoji: "\u{1F321}\uFE0F" }
    ] : [
      { keyword: "Pacing", emoji: "\u{1F4C8}" },
      { keyword: "Heart Rate", emoji: "\u2764\uFE0F" },
      { keyword: "Power Analysis", emoji: "\u26A1" },
      { keyword: "Training Load", emoji: "\u{1F3CB}\uFE0F" },
      { keyword: "Power-to-Weight", emoji: "\u{1F4AA}" },
      { keyword: "Climbing", emoji: "\u26F0\uFE0F" },
      { keyword: "Cadence", emoji: "\u{1F504}" },
      { keyword: "Gradient", emoji: "\u{1F4D0}" },
      { keyword: "VAM", emoji: "\u{1F9D7}" },
      { keyword: "Torque", emoji: "\u{1F527}" },
      { keyword: "Segment", emoji: "\u{1F3C5}" }
    ];
  ...
  if (crunched.segments_summary?.prs > 0) {
    lines.push(``);
    lines.push(`\u{1F3C5} ${crunched.segments_summary.prs} personal record${crunched.segments_summary.prs > 1 ? "s" : ""}!`);
  }
  lines.push(``);
  lines.push(`\u{1F4CA} Analyzed with Strava Analytics`);
  return lines.join("\n");
}
function buildPrivateNotes(crunched, analysisText) {
  const lines = [];
  if (analysisText) {
  ...
  lines.push(`\u{1F4CB} KEY STATS`);
  if (crunched.surf_analysis) {
    const sa = crunched.surf_analysis;
    lines.push(`\u{1F30A} Waves: ~${sa.wave_count} | Max: ${sa.max_wave_speed_kmh} km/h`);
    lines.push(`\u{1F3C4} Ride: ${sa.ride_pct}% | Paddle: ${sa.paddle_pct}%`);
  }
  if (crunched.heart_points) {
    lines.push(`\u{1F49A} Heart Points: ${crunched.heart_points.points} / 150 weekly`);
  }
  ...
  if (!analysisText) {
    lines.push(``);
    if (crunched.cadence?.is_low) lines.push(`\u{1F4A1} Cadence low (${crunched.cadence.stats.avg} rpm) \u2014 try 80-85 on flats`);
    if (crunched.heart_rate?.cardiac_drift?.drift_bpm > 5) lines.push(`\u{1F4A1} HR drift +${crunched.heart_rate.cardiac_drift.drift_bpm}bpm \u2014 start easier`);
  }
  return lines.join("\n");
}
...
function sanitizeAIOutput(text) {
  ...
  const marker = "## \u{1F3C6} POGA\u010CAR SCORE";
...
async function fetchLatestActivities(client, count) {
  await rateLimitDelay();
  const response = await client.get("/athlete/activities", { params: { per_page: count, page: 1 } });
  return response.data;
}
function buildStreamTable(enriched) {
  const s = enriched.streams;
  const timeData = s.time?.data;
  if (!timeData || timeData.length === 0) return [];
  const rows = [];
  for (let i = 0; i < timeData.length; i++) {
    const row = { time_seconds: timeData[i] };
    if (s.distance) row["distance_meters"] = s.distance.data[i];
    if (s.velocity_smooth) {
      const speed = s.velocity_smooth.data[i];
      row["speed_ms"] = speed;
...
async function callAI(provider, apiKey, model, instructions, data) {
  const { default: axios2 } = await Promise.resolve().then(() => (init_axios2(), axios_exports));
  const userMsg = `Write the full activity analysis based on this pre-computed data. All math is done \u2014 just interpret and write:

${data}`;
  if (provider !== "gemini") {
    const url2 = PROVIDER_URLS[provider] || PROVIDER_URLS.openai;
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        const res = await axios2.post(url2, { model, messages: [{ role: "system", content: instructions }, { role: "user", content: userMsg }], temperature: 0.4, max_tokens: 8e3 }, { headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" }, timeout: 18e4 });
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      const res = await axios2.post(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`, { system_instruction: { parts: [{ text: instructions }] }, contents: [{ parts: [{ text: userMsg }] }], generationConfig: { temperature: 0.4, maxOutputTokens: 8e3 } }, { headers: { "Content-Type": "application/json" }, timeout: 18e4 });
      const c = res.data.candidates;
      if (!c || c.length === 0) throw new Error("No candidates");
      return c[0].content.parts.map((p) => p.text).join("");
    } catch (err) {
      const status = err.response?.status;
      if ((status === 429 || status === 503) && attempt < 3) {
        let wait = status === 503 ? 30 : 60;
        try {
          const d = err.response.data?.error?.details?.find((d2) => d2["@type"]?.includes("RetryInfo"));
          if (d?.retryDelay) wait = Math.ceil(parseFloat(d.retryDelay.replace("s", ""))) + 5;
        } catch {
        }
...
function displayActivities(activities) {
  ...
  for (let i = 0; i < activities.length; i++) {
    ...
    const dist = (a.distance / 1e3).toFixed(2).padStart(9);
...
async function main() {
  ...
  if (rider.weightKg) console.log(`   \u2696\uFE0F Rider: ${rider.weightKg} kg | FTP: ${rider.ftpW || "?"} W | Max HR: ${rider.maxHr || "?"} bpm`);
  while (true) {
    console.log("\u{1F4CB} Fetching your last 10 activities...\n");
    let activities = await fetchLatestActivities(client, 10);
...
\u{1F449} Pick activity (1-${activities.length}), 'more' for 20, or 'q' to quit: `);
    ...
    if (input.toLowerCase() === "more") {
      activities = await fetchLatestActivities(client, 20);
      displayActivities(activities);
      const input2 = await prompt(`
\u{1F449} Pick activity (1-${activities.length}), or 'q' to quit: `);
      if (input2.toLowerCase() === "q") {
        console.log("\u{1F44B} Bye!");
        return;
      }
      const n2 = parseInt(input2, 10);
      if (isNaN(n2) || n2 < 1 || n2 > activities.length) {
        console.log("\u274C Invalid.");
        continue;
      }
      selected = activities[n2 - 1];
    } else {
    ...
    console.log(`
${"\u2550".repeat(60)}`);
    console.log(`  \u{1F4E5} STEP 1/4: Fetching "${selected.name}"...`);
...
`);
    ...
    if (!(0, import_fs.existsSync)(OUTPUT_DIR)) (0, import_fs.mkdirSync)(OUTPUT_DIR, { recursive: true });
    const outputPath = (0, import_path.join)(OUTPUT_DIR, `activity_${activityId}_${dateStr}_${safeName}.json`);
    (0, import_fs.writeFileSync)(outputPath, JSON.stringify(exportData, null, 2), "utf-8");
    console.log(`\u2705 Saved: ${(0, import_path.basename)(outputPath)} (${(Buffer.byteLength(JSON.stringify(exportData)) / 1024 / 1024).toFixed(2)} MB, ${streamTable.length} stream points)`);
    console.log(`
${"\u2550".repeat(60)}`);
    console.log(`  \u{1F52C} STEP 2/4: Crunching ${streamTable.length} data points...`);
    console.log(`${"\u2550".repeat(60)}
`);
    const crunched = crunchActivity(exportData, rider);
    if (!(0, import_fs.existsSync)(ANALYSIS_DIR)) (0, import_fs.mkdirSync)(ANALYSIS_DIR, { recursive: true });
    const crunchedPath = (0, import_path.join)(ANALYSIS_DIR, `activity_${activityId}_${dateStr}_${safeName}_crunched.json`);
    (0, import_fs.writeFileSync)(crunchedPath, JSON.stringify(crunched, null, 2), "utf-8");
    console.log(`\u2705 Crunched: ${Math.round((0, import_fs.readFileSync)(crunchedPath).length / 1024)} KB (zero sampling)`);
    if (crunched.pogacar_score) console.log(`   \u{1F3C6} Poga\u010Dar Score: ${crunched.pogacar_score.composite_pct}% (${crunched.pogacar_score.reference})`);
    ...
    if (crunched.power_to_weight) console.log(`   \u{1F4AA} W/kg: ${crunched.power_to_weight.avg_wkg} avg`);
    console.log(`   \u{1F4C8} Pacing: ${crunched.pacing.type}`);
    const aiConfigs = loadAIConfig();
    let analysisText = null;
    if (aiConfigs) {
      console.log(`
${"\u2550".repeat(60)}`);
      console.log(`  \u{1F916} STEP 3/4: AI analysis (${aiConfigs.map((c) => c.provider.toUpperCase()).join(" \u2192 ")})...`);
      console.log(`${"\u2550".repeat(60)}
`);
      ...
      for (let i = 0; i < aiConfigs.length; i++) {
        ...
        try {
          console.log(`\u{1F916} Trying ${cfg.provider.toUpperCase()} (${cfg.model})...`);
          analysisText = sanitizeAIOutput(await callAI(cfg.provider, cfg.apiKey, cfg.model, instructions, data));
          const analysisPath = (0, import_path.join)(ANALYSIS_DIR, `activity_${activityId}_${dateStr}_${safeName}_analysis.md`);
          (0, import_fs.writeFileSync)(analysisPath, analysisText, "utf-8");
          console.log(`\u2705 AI analysis saved: ${(0, import_path.basename)(analysisPath)}`);
          break;
        } catch (err) {
          const next = i + 1 < aiConfigs.length ? aiConfigs[i + 1] : null;
          if (next) {
            console.log(`\u26A0\uFE0F  ${cfg.provider.toUpperCase()} failed: ${err.message}`);
            console.log(`   \u{1F504} Falling back to ${next.provider.toUpperCase()} (${next.model})...`);
          } else {
...
\u26A0\uFE0F  STEP 3/4: Skipped (no AI API keys in .env)`);
    ...
    console.log(`
${"\u2550".repeat(60)}`);
    console.log(`  \u{1F4E4} STEP 4/4: Updating Strava activity...`);
    console.log(`${"\u2550".repeat(60)}
`);
    ...
    try {
      ...
      console.log(`   \u{1F512} Private notes: ${privateNotes.length} chars (short tips)`);
      console.log(`
\u{1F517} https://www.strava.com/activities/${activityId}`);
    } catch (err) {
...
${"\u2550".repeat(60)}`);
    console.log(`  \u2705 DONE!`);
    console.log(`${"\u2550".repeat(60)}`);
    console.log(`  \u{1F4E5} Activity: ${outputPath}`);
    console.log(`  \u{1F52C} Crunched: ${crunchedPath}`);
    if (analysisText) console.log(`  \u{1F916} Analysis: analysis/...analysis.md`);
    console.log(`  \u{1F4E4} Strava:   https://www.strava.com/activities/${activityId}`);
    console.log(`${"\u2550".repeat(60)}
`);
  }
}
