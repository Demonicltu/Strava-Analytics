...
function displayActivities(activities: SummaryActivity[]) {
...
async function main() {
  ...
  while (true) {
    console.log("📋 Fetching your last 10 activities...\n");
    ...
    const input = await prompt(`\n👉 Pick activity (1-${activities.length}), 'more' for 20, or 'q' to quit: `);
    if (input.toLowerCase() === "q") { console.log("👋 Bye!"); return; }

    let selected: SummaryActivity;
    if (input.toLowerCase() === "more") {
      activities = await fetchLatestActivities(client, 20);
      displayActivities(activities);
      const input2 = await prompt(`\n👉 Pick activity (1-${activities.length}), or 'q' to quit: `);
      if (input2.toLowerCase() === "q") { console.log("👋 Bye!"); return; }
      const n2 = parseInt(input2, 10);
      if (isNaN(n2) || n2 < 1 || n2 > activities.length) { console.log("❌ Invalid."); continue; }
